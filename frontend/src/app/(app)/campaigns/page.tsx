...
function isCampaignLive(campaign: Campaign) {
  if (campaign.status === "sending" || campaign.status === "sent") return true;

  if (campaign.status !== "completed" || !campaign.sent_at) return false;

  const sentAt = parseBackendDate(campaign.sent_at).getTime();
...
export default function CampaignsPage() {
  ...
  useEffect(() => {
    const hasActiveCampaigns = campaigns.some(isCampaignLive);
  ...
  const handleSendCampaign = async (campaignId: string) => {
    ...
    try {
      ...
      alert(getErrorMessage(error, "Failed to send campaign"));
    } finally {
      setSendingId(null);
    }
  };
    ...
    <div className="animate-fade-in">
      ...
      {campaigns.some(isCampaignLive) && (
        <div style={{ textAlign: "center", padding: "16px 0", color: "var(--text-muted)", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
          ...
          Live updating â€” delivery receipts arriving from channel service
        </div>
      )}
    </div>
  );
